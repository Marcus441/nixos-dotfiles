...
# (c) 2018-2023 bat-developers) as data, rendered to plist by nixpkgs rather
# than carried as 540 lines of XML.
{
  pkgs,
  colors,
}: let
...
in
  pkgs.writeText "kanagawa-dragon.tmTheme" (pkgs.lib.generators.toPlist {escape = true;} {
    ...
    settings = [
      ...
      # `none` matches nothing. Upstream carries these to record which slot each
      # kind would take if a grammar ever scoped it; kept for the same reason.
