...
For structural changes, prove nothing changed but order. The host list is
`docs/inventory.md`, never a literal here:
...
builds the current tree only). A target for another platform — a darwin host's
system and home on a Linux builder — cannot be built here, so `verify.sh`
compares it by `nix eval --raw '<ref>#<attr>.outPath'`: the output path of an
input-addressed derivation is fixed by evaluation, so the signature is the one
`nix build` would print. Such a row reads `PASS (eval)`, and `diff-closures`
is not available for it — a FAIL there is explained by reading the diff of the
evaluated config, not the closure.
