...
<a id="lazygit"></a>
## `git/lazygit.nix`, `dev/lazydocker.nix` — `selectedLineBgColor = ["default"]`
...
no `removePrefix`, no `colorsRgb`, no `esc` helper. The module used to name ANSI
colours (`"blue"`, `"magenta"`), which meant the palette could move without it.
The swatch row is the deliberate exception: it draws SGR 40–47/100–107, i.e.
`desktop.ansi` as the terminal itself resolves it.
...
ASCII and the `─` run is generated by count, never measured. The box has no
right edge because fastfetch aligns keys (`display.key.width`, emitted as an
absolute-column escape) but has no matching width for values, and the values are
dynamic — a closing `│` could not be placed.
**Breaks** *Silently.* A rule that no longer spans its widest value reads as a
...
for modules that specify no format. 11 is bar (2) + number (1) + colour (8),
which is what `memory` and `disk` ask for explicitly.
**Breaks** *Silently.* At the default 9 the placeholder renders as an empty
string, so the line loses its bar and keeps its spacing.
