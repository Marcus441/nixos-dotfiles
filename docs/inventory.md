# Inventory

Generated by `scripts/inventory.sh`. Do not edit; do not restate any of
it in prose. `scripts/docs-check.sh` fails when this file is stale.

## Hosts

Aspect order is load-bearing — it sets merge order, which reaches
derivation hashes.

| Host | Aspects |
| --- | --- |
| `gpc` | `core kitty zsh gaming nvidia hyprland tearing quickshell thunar apps` |
| `swift5` | `dev core foot laptop dwl dwl-bar keychron` |
| `UM790pro` | `dev core kitty zsh hyprland quickshell yazi apps keychron` |

## Aspects

Enumerate the files behind any row with
`rg -l "flake\.modules\.<class>\.<aspect>" modules/`.

| Aspect | Classes | Files |
| --- | --- | --- |
| `alacritty` | `homeManager nixos` | 2 |
| `apps` | `homeManager nixos` | 15 |
| `core` | `homeManager nixos` | 65 |
| `dev` | `homeManager nixos` | 6 |
| `dwl` | `homeManager nixos` | 11 |
| `dwl-bar` | `homeManager nixos` | 2 |
| `foot` | `homeManager nixos` | 2 |
| `gaming` | `homeManager nixos` | 11 |
| `ghostty` | `homeManager nixos` | 2 |
| `hyprland` | `homeManager nixos` | 22 |
| `keychron` | `nixos` | 1 |
| `kitty` | `homeManager nixos` | 2 |
| `laptop` | `homeManager nixos` | 3 |
| `nvidia` | `homeManager nixos` | 4 |
| `quickshell` | `homeManager` | 3 |
| `tearing` | `homeManager` | 1 |
| `thunar` | `homeManager nixos` | 2 |
| `walker` | `homeManager` | 4 |
| `yazi` | `homeManager` | 1 |
| `zsh` | `homeManager nixos` | 3 |

## Aspect requirements

Declared in the file that creates the dependency, never in a central table.

| Aspect | Requires | Declared in |
| --- | --- | --- |
| `dwl-bar` | `dwl` | `bar/dwl-bar.nix` |
| `quickshell` | `hyprland` | `quickshell/quickshell.nix` |
| `tearing` | `gaming hyprland` | `gaming/tearing.nix` |
| `walker` | `hyprland` | `launcher/walker.nix` |
| `yazi` | `apps` | `filemanager/yazi.nix` |

## Counts

| | |
| --- | --- |
| files declaring at least one aspect | 123 |
| of those, declaring more than one aspect or class | 35 |
| `load-bearing:` pointers | 128 |
| anchors defined in `docs/` | 108 |
